...
class MGFileUtils {
    ...
    companion object {
        ...
        private fun writeFile(text: String, name: String, path: File?): Boolean {
            ...
            MGLogUtils.d("寫入檔案: ${outputFile.absolutePath}")
            ...
            try {
                ...
                MGLogUtils.d("寫入過程發生錯誤: ${e.message}")
            }
            return true
        }
        ...
        private fun readFile(name: String, path: File?): String? {
            ...
            MGLogUtils.d("讀取檔案: ${inputFile.absolutePath}")
            //先檢查路徑是否有檔案存在
            if (!inputFile.exists() || !inputFile.isFile) {
                MGLogUtils.d("路徑下無檔案存在: ${inputFile.absolutePath}")
                return null
            }
        ...
        private fun isWriteConditionOK(path: File?): Boolean {
            if (path == null) {
                //尚未初始化, 不能寫入檔案
                MGLogUtils.w("路徑null, 不能寫入檔案")
                return false
            }

            if (MGThreadUtils.isMain()) {
                //主線程, 不行寫入讀取, 退出
                MGLogUtils.w("主線程, 不能寫入檔案")
                return false
            }
